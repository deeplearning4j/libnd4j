...
namespace helpers {
    ...
    FORCEINLINE Nd4jStatus _dilation_hw(int *in, int *wh, std::vector<int> &strides, std::vector<int> &rates, bool isSameMode, int *stride_rows, int *stride_cols, int *rate_rows, int *rate_cols, int *pad_top, int *pad_left, int *out_rows, int *out_cols) {
        ...
        *rate_cols = rates[2];
        ...
        _outputSize(input_rows, filter_rows_eff, *stride_rows, isSameMode, out_rows, pad_top, padding_after_unused, padding_after_unused);
        _outputSize(input_cols, filter_cols_eff, *stride_cols, isSameMode, out_cols, pad_left, padding_after_unused, padding_after_unused);
