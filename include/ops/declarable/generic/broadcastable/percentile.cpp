...
CUSTOM_OP_IMPL(percentile, 1, 1, false, 1, -2) {    
    ...
    NDArray<T> *input  = INPUT_VARIABLE(0);                                             // tensor with rank > 0
    ...
    REQUIRE_TRUE((T)0. <= q && q <= (T)100., 0, "PERCENTILE OP: percentile parameter must be within [0, 100] range, but got %f instead !", q);
