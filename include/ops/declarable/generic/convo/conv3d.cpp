...
CUSTOM_OP_IMPL(conv3dnew, 2, 1, false, 0, 13) {
    ...
    int isValidMode = INT_ARG(12);                                              // 0-SAME,  1-VALID
    int isNCDHW  = block.getIArguments()->size() > 13 ? INT_ARG(13) : 0;        // 0-NDHWC, 1-NCDHW    
...
DECLARE_SHAPE_FN(conv3dnew) {
    ...
    ConvolutionUtils<T>::calcOutSizePool3D(oD, oH, oW, kD, kH, kW, sD, sH, sW, pD, pH, pW, dD, dH, dW, iD, iH, iW, isValidMode);
...
CUSTOM_OP_IMPL(conv3dnew_bp, 3, 2, false, 0, 13) {
    ...
    ConvolutionUtils<T>::calcOutSizePool3D(trueoD, trueoH, trueoW, kD, kH, kW, sD, sH, sW, pD, pH, pW, dD, dH, dW, iD, iH, iW, isValidMode);    
